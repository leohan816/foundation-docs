...
> **버전: v0.2 · 2026-07-03 (v0.1 초안 + 적대 리뷰 4개 렌즈 반영, fact base 재검증)**
> **release train 단계: 설계 → (승인 대기) → implementation prompt → repo-local 구현 → cross-project regression → report**
...
- **Cosmile (★이미 furef 전송 — anonymous 아님):** `Cosmile/app/src/app/api/slice/consult-foundation/route.ts:42-43`
  ...
  = **32 hex(128-bit) · secret 있으면 HMAC-SHA256, 없으면 SHA256(fail-open).** 식별자 없으면 `null`(anonymous consult 유지).
...
3. **모든 서비스는 Foundation 호출 시 `foundation_user_ref`를 전송한다**(SIASIU·Cosmile·신규 서비스 공통). 식별자 없으면 명시적 `null`(anonymous consult 허용).
...
### F-3. 표준 방향 (보안 강화 — 승인 결정 필요)

권장: plain SHA256 대신 **HMAC-SHA256 + 단일 server-side secret**.
...
6. **권장 알고리즘 = HMAC-SHA256 + server-side secret**(plain SHA256은 과도기·표준 미충족).
7. **절단 길이 = 최소 128-bit(`[:32]` hex)** 로 통일. 64-bit(`[:16]`)는 충돌 위험(§M-2)으로 표준 미달.
8. **Foundation `subject_ref`도 128-bit로 상향**(현재 `[:16]` = 64-bit가 실제 memory 키이므로 이 상향이 furef 상향보다 우선).

★**단, 알고리즘/길이 통일은 memory continuity 위험(§M)을 수반하므로 반드시 별도 release train·승인 후 진행.** 본 문서는 방향만 고정한다.
...
이 정책은 아래 결정이 **승인되기 전에는** implementation prompt를 발행하지 않는다.

- **D-1 (역추적 저항 불변식 확정):** "역추적 불가(enumeration-resistant)"를 표준 필수로 유지하려면 **HMAC-SHA256 + server-side secret을 required로 승격**하고 secretless 경로를 "shadow/과도기 한정·표준 미충족"으로 못박는다 — **또는** plain SHA256 경로의 속성을 "hash one-wayness only(NOT enumeration-resistant)"로 공식 다운그레이드한다. **둘 중 하나를 Leo가 확정.** (본 문서는 전자를 권장하되 결정을 열어 둔다.)
- **D-2 (통일 시점):** SIASIU/Cosmile 알고리즘·길이·subject-type 통일을 **Memory live 전에** 완료할지, dual-write migration으로 미룰지.
- **D-3 (절단 길이):** furef·subject_ref 표준 길이 = 128-bit(`[:32]`) 확정 및 `_SALT` freeze 시점.
- **D-4 (guest 정책):** guest = session-only(non-persistent) 확정 여부, guest→login reconciliation을 별도 consent release train으로 둘지.

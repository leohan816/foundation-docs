...
> 상태: **DESIGN** (통합본 · 승인 전 · Control 검증 대기)
> 버전: v0.2-integrated · 작성일: 2026-07-04 · 선행: v0.1-draft + 적대 리뷰 3축(ownership-boundary / pii-deanon / same-structure-completeness) 반영
...
> **design-first 전제:** 본 문서는 **구현 전 설계**다(CLAUDE.md §2.6). PASS/APPROVED 전에는 implementation prompt 미발행 · FOUNDATION/SIASIU/Cosmile 코드 미수정.
...
- **공통 계약이 목표.** SIASIU를 reference로, Cosmile를 같은 **Common Service Memory Contract**로 정렬(결정 #1~#4). "동일 계약"의 정의를 **동일 필드-의미/상태머신/게이트 + 정본 위치는 서비스-소유**로 확정한다(대칭성 리뷰 반영, §E/§I-3/§B).
- **memory_context는 요청마다 생성 → consult 중 임시 사용 → 즉시 폐기.** opaque refs·enum·atom·consent flags만 담고, **raw 상담원문 · raw PII · 주문/결제/배송은 제외**한다.
- **identity/de-anon.** furef_local_ref/subject_ref는 **가명(pseudonymous) 개인정보**다 — 익명이 아니며 삭제·보존·동의 lifecycle 대상(§M/§N/§S). secretless SHA256 furef(SIASIU 현행 16-hex plain SHA256)은 **열거 가능 → 원 식별자 복원 가능**하므로 recoverable pseudonymous PII로 취급한다. cross-service linkage는 **consent fail-closed + broker flag OFF로 차단 중**이며 "구조적으로 불가"가 아니다.
...
| # | 결정 | 설계 귀결 |
|---|------|-----------|
| 1 | SIASIU·Cosmile 동일 Service Memory Contract 구현 | §E 공통 계약 1벌 정의 → 양 서비스가 각자 로컬 storage로 구현. **"동일 계약" = 동일 필드-의미/상태머신/게이트/거버넌스 오버레이 공유. 정본(원문·commerce)의 물리 위치는 서비스-소유이며 비대칭 허용**(아래 #2·#3 참조) |
| 2 | 둘 다 상담 히스토리 장기보관 + 고객 재열람 | §I ConversationSession/Message/EpisodeSummary. **원문 정본 위치는 서비스-소유** — opt-A 채택 시 conversation 축은 원문 소유가 비대칭(SIASIU=원문 정본, Cosmile=재열람 read surface + SIASIU/Consultation 링크). 재열람 기능은 양쪽 제공. **opt-A/opt-B는 M1 PASS 전 확정**(§I-3) |
...
| 10 | cross-service 고객기억 공유 기본 OFF · consent-gated broker만 | §N consent 4층 + §M FoundationSyncState + shared_memory gate(default OFF) |
...
- **B-e. (신규) guest → subject 신원확정 병합 규칙.** 게스트가 로그인/신원확정될 때 `guest_ref`/`anonymousId` 소유 레코드(ConversationSession·LongTermMemoryFact·CommerceMemory·consent)를 subject_ref로 **재키잉/동의승계/병합**하는 절차를 정의한다(§M-2/§N-5). 미동의(allow_link=false) 시 병합 거부. anonymousId(device-level)→subject_ref(furef 파생) 매핑도 이 결정에 포함. (§W-W22 WATCH)
...
★정정(적대 리뷰 high/medium): 초안의 "Foundation 요청 경로에는 고객 식별자가 없으므로 de-anon join이 **구조적으로 불가**"는 **부정확**하다. 정확한 서술 = **consult 요청 경로는 오늘 identity 필드를 싣지 않으며, cross-service linkage는 consent fail-closed + broker flag OFF로 차단 중**(구조적 불가가 아님). session_context는 untyped freeform dict이므로 서비스가 식별자/원문/주문을 넣으면 Foundation은 수용·반영한다. 따라서 **경계 강제는 §R의 ingress default-deny scan**(FOUNDATION `shared_memory/gate.has_raw_or_pii` 재사용)이 담당해야 하며, §V에서 `session_context 내 raw/PII = 0`을 assert한다. broker를 켜는 것(§R)은 **오늘 identity 토큰이 없는 경로에 identity를 배선하는 행위** = 게이팅해야 할 위험 순간이다.
...
- **Conversation 축:** 원문 정본 위치 서비스-소유(opt-A 시 SIASIU=원문 정본, Cosmile=read surface+링크 → 원문 소유 비대칭 허용). §I-3에서 opt-A/opt-B 확정.
...
- Cosmile 상담 원문 정본은 계속 SIASIU/Consultation DB 링크(`consultationSessionId`)로 둔다 — 자체 원문 신설 여부는 §I-3 opt-A/opt-B.
...
| external_consult_ref | TEXT | idx | Y | → SIASIU/Consultation DB | Cosmile↔SIASIU 링크(consultationSessionId 대응) |
| consent_scope | TEXT | | N | enum(§N) | 세션 기본 동의 범위 |
| retention_policy | TEXT | | N | enum(§N) | 기본 standard_ttl |
| sensitivity_default | TEXT | | N | (normal\|sensitive\|high) | |
...
**★Cosmile opt-A/opt-B 확정(M1 PASS 전 결정 — 적대 리뷰 medium 반영):**
- (opt-A) Cosmile는 ConversationMessage 원문을 자체 신설하지 않고 EpisodeSummary + external_consult_ref(SIASIU DB) 링크만 유지. → **conversation 축 원문 소유가 비대칭**(SIASIU=원문 정본, Cosmile=재열람 read surface + 링크). 이 경우 결정 #1의 "동일 계약"은 **필드-의미/상태머신/게이트 대칭 + 정본 위치 서비스-소유**로 충족되며, 원문 물리 중복/원장 이원화를 피한다.
- (opt-B) Cosmile도 자체 원문 저장 신설. → 결정 #2를 문자 그대로 대칭 충족하나 원문 정본이 이원화된다.
- **권장 = opt-A**(원문 단일 정본 = SIASIU/Consultation + Cosmile 재열람 read surface). **단, opt-A는 conversation 축 비대칭을 명시적으로 수용하는 것**이므로 §B 결정 #1/#2의 "대칭" 단정을 위 재정의로 완화한다. **어느 옵션인지 M1 PASS 전 Leo 승인으로 확정.**
...
| confidence | REAL | | N | | |
| fact_state | TEXT | | N | (hypothesis\|active\|superseded) | 2번 규칙 |
...
|---|---|---|---|---|---|
| id | INT/TEXT | PK | N | — | 유지 |
| subject_ref | TEXT | idx | N | → SubjectRefMap | ★신규(user_id 대체 keying·**가명 PII**) |
...
| sensitivity_level | TEXT | | N | (normal\|sensitive\|high) | ★신규 |
...
| cross_service_broker | BOOL | | N | =False | ★기본 OFF(결정 #10) — consent-gated broker만 True |
...
- **SubjectRefMap = subject_ref + furef_local_ref + service_id를 한 row에 co-locate하는 문자 그대로의 cross-service join table**이므로, LongTermMemoryFact와 **동일한 consent/retention/delete governance** 아래 둔다(erasable·consent-gated).
...
- 따라서 **'v2 HMAC + prod fail-closed + 하드코딩 salt 제거'는 cross-service broker enable의 hard precondition**(가정된 속성이 아님). broker-ON 시 §V에서 "평문 local_ref 거부"를 assert.
...
### N-4. Cross-store Erasure 전파(신설 계약)

right-to-erasure 3-store 일관성: 삭제 요청 → (1) SIASIU memory.db, (2) Cosmile prisma, (3) Foundation shadow store 세 곳 + 파생물(content_hash/summary/evidence_refs) + **SubjectRefMap 매핑** 폐기까지 전파. 현재 delete는 Foundation shadow store subject_ref 플래그만. 본 계약 정의 항목: 전파 순서·idempotency·audit·revocation(consent withdrawal→block→delete) 흐름. **live 전파는 별도 release train**(본 설계는 shadow 시뮬레이션까지).

### N-5. Guest → Subject 병합(신규, B-e 상술)

게스트 로그인/신원확정 시:

1. **재키잉:** `guest_ref`/`anonymousId_ref` 소유 레코드(ConversationSession·LongTermMemoryFact·CommerceMemory·EpisodeSummary·ConsentRecord)를 subject_ref로 재키잉.
2. **consent 승계/재확인:** 게스트 시점 consent를 subject로 승계하되 sensitive/cross_service는 재확인.
3. **idempotency·audit:** 병합은 idempotent(중복 실행 무해)하고 audit 기록.
4. **미동의 거부:** `allow_link=false`면 병합 거부(매핑 미생성, guest 레코드 분리 유지).

★anonymousId(device-level)→subject_ref(furef 파생) 매핑도 이 규칙을 따른다(§W-W22).
...
- **★ingress default-deny scan(신규 · 적대 리뷰 high 반영):** consult_contract 진입 시 session_context/service_context/product_context를 **FOUNDATION `shared_memory/gate.has_raw_or_pii` 재사용**해 스캔하고, raw utterance/PII/고객 식별자 발견 시 **fail-closed reject**한다(현재 이 gate가 foundation_http_service에 미배선 — M5에서 배선). §V에서 `session_context 내 raw/PII/식별자 = 0` assert.
...
**관계:** P1/P2/P3는 본 공통 계약이 **정본 구조로 예방**해야 할 실재 de-anon/PII 표면이다. **★§S row1의 'trace_id de-anon 불가'는 Foundation-side에 한정된 진술**이며, **서비스-side persistence(trace_id를 identity 옆에 저장)는 명백한 de-anon 벡터**임을 명시한다. 각 patch의 실제 코드 수정은 **해당 repo 소관 · 별도 release train**(control tower가 contract/plan 발행 → repo-local 구현). 본 설계는 patch가 정합해야 할 계약(opaque keying · payload refs only · sanitize · trace_id hashing/미저장 · identity 필드 분리)을 고정한다.
...
- **M1 — Scope 확정 (본 문서 승인).** Foundation memory scope(§C/§D) · 공통 계약 5축(§E) · taxonomy 정합(§N) · **opt-A/opt-B 확정(§I-3)** · **furef/subject_ref 가명 PII 분류(§M/§S)** 확정. 산출: 본 설계서 PASS/APPROVED. 검증: Leo 승인 · 정본 audit·identity policy 정합.
...
- **M5 — Foundation consult 연결 (flag OFF).** **★ingress default-deny scan(has_raw_or_pii 재사용) 배선**(§R) · subject_ref bridge·memory_reuse_decision(FRC) 후속 배선은 shadow·default OFF. **cross_service_broker precondition: v2 HMAC + prod fail-closed + 하드코딩 salt 제거 + 평문 local_ref reject 배선**. 검증: OFF면 API inert · ingress가 raw/PII/식별자 reject · memory_read_provider_called=False · Foundation runner 89/89·651.
- **M6 — live 게이트 (별도 release train · 미포함).** consent ledger live · cross-store erasure live 전파 · cross-service broker enable · guest 병합 live = 각각 별도 승인. 본 설계 범위 밖.
...
- **ingress gate:** session_context/service_context/product_context 내 **raw utterance/PII/고객 식별자 = 0**(has_raw_or_pii reject 동작 assert). ★freeform dict에 raw 삽입 시 reject 확인.
...
| W11 | cross-store erasure 전파(SIASIU/Cosmile/Foundation 3-store + 파생물 + SubjectRefMap 폐기) 미정의 | WATCH(별도 train) | shadow subject_ref 플래그만 |
...
| W16 | subject_ref v2 hard gate(c9bb996)가 foundation_http_service에 미배선(unpushed·flag OFF·consult 미소비) | **PATCH_REQUIRED-before-broker**(격상) | core/contracts/server grep 0 · SUBJECT_REF_HARD_GATE_RESULT_20260704 |
...
| **W22** | **guest_ref/anonymousId → subject_ref 병합(재키잉·consent 승계·idempotency·미동의 거부) 미정의(B-e로 도입)** | **PATCH_REQUIRED(신규)** | §M-2/§N-5 · CommerceEvent.anonymousId |
...
> **완료/미완/금지 구분**
> **완료(설계):** Foundation memory scope 고정 · 공통 계약 5축(동일 계약 정의 재확정) · memory_context 정의/schema/lifecycle · consent/retention/delete 구조(is_safety override·guest 병합) · de-anon guard(가명 PII 재분류·ingress gate·keyed hash·trace_id scope) · P1/P2/P3 canonical fix 복원 · 구현 단계.
> **아직 아님:** M2~M6 구현 · ingress gate 배선 · consent ledger · cross-store erasure · Cosmile 신규 모델 · SubjectRefMap governance · guest 병합 · FRC memory_reuse_decision · retention TTL 기간 확정 · opt-A/opt-B 최종 확정.

...
|---|---|---|---|---|---|---|
| **B1** | `1ce099e` readiness adapter 소재 확인(baseline 164/164 discrepancy) | Cosmile+control | **M4 선행(schema 전)** | git 히스토리/clone 전수 조사 | 소재 확정·164/164 재현 | 소재 불명 시 M4 Cosmile schema 착수 보류 |
...
| **B4** | Foundation ingress **신규 default-deny gate**(M3 §7 v1.2) | Foundation | **M4(shadow)·M5(consult 배선)** | M3 §7 스펙 | raw/PII/식별자 reject·echo round-trip pass | gate flag OFF(inert) |
| **B5** | opt-B raw at-rest 보안(§11 6항 exit criteria) | Cosmile+SIASIU | **M4(스펙 확정)·M6(raw live)** | §11 스펙 문서 | 암호화·키/rotation·접근·감사·보존·파기 | raw 저장 flag OFF |
...
| **B8** | retention TTL 실기간·expires_at·auto-sweep(임신 max-age) | control+서비스 | **M4(값 확정)·M5(sweep)** | TTL 표 확정 | expired must_not_reappear·is_safety override | flag OFF |
...
| **B14** | consent write-gate+backfill(raw 저장 사전 grant) | 서비스 | **M4** | ConsentRecord ledger | grant 없으면 raw 저장 0/session-only·backfill default | flag OFF |
...
- **P1/P2 de-anon 동반패치(B2):** P1 foundation_decision_received(userId+foundation_trace_id 동일 row 분리·trace_id keyed-hash/미저장)·P2 FoundationSignalOutbox(raw ids→payload_refs).
...
- **backup:** SIASIU memory.db·Cosmile prisma dev.db migration 전 백업.
...
3. **Foundation gate shadow**(B4 ingress gate·B3 subject_ref 배선·durable write 0).

...
> depends_on: [COSMILE_MEMORY_V3_DATA_DICTIONARY_CANONICAL_20260706.md, COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md, COSMILE_MEMORY_V3_07_SAFETY_ADVERSE_REACTION_GUARDRAIL_20260706.md] · owns: [catalog intelligence mapping(product/ingredient/skin_goal/sensitivity/claim 구조 계약), **margin_band 산출**(band 판정 기준 — 정확 원가 비저장), claim vs evidence 분리 계약(evidence_grade)] · referenced_by: [COSMILE_MEMORY_V3_00_INDEX_AND_EXECUTIVE_SUMMARY_20260706.md, COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md, COSMILE_MEMORY_V3_07_SAFETY_ADVERSE_REACTION_GUARDRAIL_20260706.md, COSMILE_MEMORY_V3_09_ANALYTICS_REPORT_MINIMUM_20260706.md]
...
관련 sibling 문서(실제 파일명):

- `COSMILE_MEMORY_V3_DATA_DICTIONARY_CANONICAL_20260706.md` — **enum/key/threshold 유일 정본(사전).** 본 문서는 값 목록을 재선언하지 않고 사전을 참조한다.
- `COSMILE_MEMORY_V3_00_INDEX_AND_EXECUTIVE_SUMMARY_20260706.md` — V3 전체 loop / 정체성 · Memory V1 상속 개요
- `COSMILE_MEMORY_V3_03_RECOMMENDATION_EVENT_CONTRACT_20260706.md` — 추천 이벤트·얕은 상호작용(product_view/add_to_cart) 저장 소유(사전 §1.3 R-K7)
- `COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md` — checkout/order·revenue/feedback outcome 이벤트 스키마 (본 문서의 상류)
- `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` — MemoryFactCandidate → evidence/confidence → LongTermMemoryFact 승격 (본 문서의 하류)
- `COSMILE_MEMORY_V3_07_SAFETY_ADVERSE_REACTION_GUARDRAIL_20260706.md` — Safety / adverse-reaction gate (본 문서 §6 contraindication/sensitivity가 **feed**하는 대상)
...
| `margin_band` | enum — 정본 = 사전 §2.14 | Cosmile | 마진 밴드(버킷) — **산출 소유 = 본 문서 §7** | **safety보다 하위 우선순위** |
...
- **어휘 등재 규율(P5):** 성분/민감도 controlled vocabulary의 확정본은 **사전(COSMILE_MEMORY_V3_DATA_DICTIONARY_CANONICAL_20260706.md) 개정으로만** 등재·확장한다(사전 §9). 위 목록은 초기 후보 예시다. 학습된 사실로 승격될 때의 축은 사전 §2.1 `fact_target=ingredient_id` 모델을, 고객 민감도 수준 표기는 사전 §2.8 `sensitivity_level`(low/normal/sensitive/high)을 따른다.
...
| 필드 | enum | 규칙 |
|---|---|---|
| `price_band` | `low` / `mid` / `premium` / `luxury` (제안) | 실매가·원가 저장 금지, 버킷만 |
| `margin_band` | **정본 = 사전 §2.14: `low` / `medium` / `high`** + `margin_coverage`(% 별도) (구 `mid` 표기는 superseded — 사전 §2.14) | **safety/suitability 하위 우선순위 · 사용자 노출 금지** |
| `stock_status` | `in_stock` / `low_stock` / `out_of_stock` / `discontinued` (제안) | out/discontinued는 추천 필터에서 제외 |

**margin_band 산출 소유 = 본 문서(V3-05).** band 판정 기준은 **내부 카탈로그 메타**(카테고리·브랜드·공급 조건 버킷 등)로 정의하며, 판정 과정에서도 **정확 원가를 저장하지 않는다**(§10 Hard Stop 유지). `margin_coverage`(밴드 판정 가능 비율)를 함께 산출해 하류(V3-04 outcome·V3-09 analytics)에 제공한다 — 사전 §6 Option A.

**전 계약 공통 스코프(사전 §6): 정확 원가/실매가 저장 금지·band만.** V3-04 outcome 지표·V3-09 analytics 지표도 동일 스코프를 따른다(금액 기반 margin 지표는 superseded — 사전 §6·정확 line cost 기반 margin은 pre-prod gate로 이월).
...
- Foundation에 전달되는 것은 minimized `memory_context`뿐(**최소화 계약 정본 = `COSMILE_MEMORY_V3_02_LEARNING_COMMERCE_MEMORY_CONTRACT_20260706.md` 19필드 표** — 본 문서는 참조만): 예) `{skin_goal_tags, sensitivity_tags(bool/enum), evidence_grade}` 수준.

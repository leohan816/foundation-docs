...
V3 설계(V3-01 ~ V3-09)가 실제 코드/DB/테스트로 내려가기 전, 각 항목을 **PASS/FAIL/BLOCKED**로 판정한다.
...
관련 형제 문서(파일명으로 상호참조):
- `COSMILE_MEMORY_V3_01_...` — V3 개요/타겟 러닝 커머스 메모리 루프
- `COSMILE_MEMORY_V3_02_...` — subject_ref / furef mint (service-local) 계약
- `COSMILE_MEMORY_V3_03_...` — MemoryFactCandidate → LongTermMemoryFact 승격 모델
- `COSMILE_MEMORY_V3_04_...` — recommendation_id → order/feedback linkability 계약
- `COSMILE_MEMORY_V3_05_...` — Foundation memory_context 최소화 계약 (request-scoped, no raw/PII)
- `COSMILE_MEMORY_V3_06_...` — safety/adverse-reaction 우선 규칙
- `COSMILE_MEMORY_V3_07_...` — DB invariant / schema-validate 수준 정의
- `COSMILE_MEMORY_V3_08_...` — 테스트 의미 정책 매핑 (provider-independent vs DB-touch)
- `COSMILE_MEMORY_V3_09_...` — dev/shadow/readiness vs live/prod 구분

(형제 문서 파일명이 확정되면 위 접미사를 실제 파일명으로 교체한다. ★Leo 결정 필요: 형제 문서 번호/파일명 최종 확정.)
...
전체 게이트 규칙:
- **A~I 카테고리 전 항목 PASS** 여야 구현 프롬프트 발행 가능.
...
| B3 | Foundation에 넘어가는 것은 **최소화된 request-scoped memory_context** 뿐인가? | `raw_text_stored=False`, `request_scoped=True`, no raw text/PII | raw text/PII/durable 전달 (`see V3_05`) |
...
| D2 | safety/adverse-reaction 신호가 **commerce/revenue보다 우선**하는가? | 우선순위 규칙에 safety-first 명문 (`see V3_06`) | revenue 최적화가 safety 위 |
...
| D6 | adverse_reaction 필드가 **long-term 승격과 무관하게 즉시** 반영 경로를 갖는가? | 단일 안전신호도 즉시 safety 반영(승격 대기 아님) | 안전신호가 승격 큐에 묶임 |
...
| # | 검증 질문 | PASS 기준 | FAIL 신호 |
|---|-----------|-----------|-----------|
| E1 | 타겟 루프 각 단계가 **공통 상관키**로 연결되는가? | consultation→recommendation→product/SKU→view/cart/checkout/order→satisfaction/adverse/repurchase 를 잇는 `recommendation_id`(및 상관키) 정의 (`see V3_04`) | 단계 간 연결키 부재 |
...
| E5 | 링크 부재(주문 없음/피드백 없음) 케이스가 **명시적 enum**으로 표현되는가? | 예: `link_status: linked / no_order / no_feedback / expired` | null 방치·암묵 처리 |
...
| F2 | MemoryFactCandidate/LongTermMemoryFact에 **필수 invariant**가 정의됐는가? | NOT NULL/enum 도메인/FK/unique 명시 (`see V3_07`) | invariant 누락 |
| F3 | 승격 단계에 **evidence/confidence 필드**와 임계값이 있는가? | confidence 범위·evidence count·다중신호 요건 명시 | 단일 신호 승격 허용 |
| F4 | **단일 신호로 long-term memory 확정 금지**가 스키마/규칙으로 강제되는가? | 승격 규칙: `≥N evidence & confidence≥τ` (★Leo 결정 필요: N·τ 값) | 1건으로 promote |
| F5 | 삭제/만료/차단/동의 상태가 **enum**으로 표현되는가? | `memory_reuse_decision: allowed/blocked/expired/deleted/consent_required/not_available` | 상태 누락·boolean 뭉개기 |
...
| # | 검증 질문 | PASS 기준 | FAIL 신호 |
|---|-----------|-----------|-----------|
| G1 | 각 테스트가 **보호 대상 계약/경계/위험을 증명**하는가? | 테스트 ↔ invariant/safety/boundary 매핑 존재 (`see V3_08`) | 커버리지 숫자만·의미 없음 |
...
| # | 검증 질문 | PASS 기준 | FAIL 신호 |
|---|-----------|-----------|-----------|
| I1 | 설계서가 전체를 **dev/shadow/readiness**로 명시하는가? | live/prod activation 아님을 명문 (`see V3_09`) | live 완료 뉘앙스 |
...
1. A~I 카테고리 **모든 항목 PASS** (N/A는 사유 명시).
2. J STOP 조건 **0건**.
3. ★Leo 결정 필요 항목(아래 L) **전부 해소/승인**.
4. 형제 문서 상호참조 링크가 실제 파일명으로 확정.
...
- ★Leo 결정 필요: 형제 문서(V3_01~V3_09) 번호·파일명 최종 확정.
- ★Leo 결정 필요: F4 승격 임계값 — evidence 최소 건수 `N` 및 confidence 임계 `τ`.
- ★Leo 결정 필요: E5 `link_status` enum 최종 값 집합(no_order/no_feedback/expired 외 추가 여부).
- ★Leo 결정 필요: adverse_reaction 즉시 safety 반영(D6)과 승격 큐의 분리 경계 확정.

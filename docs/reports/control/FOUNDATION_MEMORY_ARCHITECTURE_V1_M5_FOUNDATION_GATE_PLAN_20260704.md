...
| deleted/blocked/expired fact reuse | reject | must_not_reappear(service-side filter+gate) | `GATE_REJECT_DELETED_REUSE` | test_reject_deleted_blocked_expired |
...
| SSC validation | `contracts.py validate_ssc`(session_context freeform 통과:72) | gate가 validate 후/전 default-deny 적용(freeform → whitelist 강제) |
...
- **logs raw payload 저장 금지:** reject reason = **enum/code만**·raw payload/PII 미기록·trace_ring(휘발·PII 0) 준수.
...
- **echo round-trip tests**(§6·previous session_context_out → gate pass·safety carry·catalog).
...
- **M6로 넘길 항목:** hard reject 활성 · prod secret live · consult live 배선(real user) · cross-service.

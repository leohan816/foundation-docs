...
관련 형제 문서(cross-reference — 실제 파일명 정본, 구 번호 참조 superseded):
- `COSMILE_MEMORY_V3_00_INDEX_AND_EXECUTIVE_SUMMARY_20260706.md` — V3 전체 루프 개요 / canonical inheritance (구 "V3-01 루프 개요" 참조 superseded).
- `COSMILE_MEMORY_V3_02_LEARNING_COMMERCE_MEMORY_CONTRACT_20260706.md` — per-customer 메모리 계약·memory_context 최소화 계약 소유(식별자/키 형식 정본 = 사전 §1).
- `COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md` — Order / Feedback / Outcome Event Contract (recommendation_id downstream 연결 대상).
- `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` — MemoryFactCandidate → LongTermMemoryFact promotion (구 "V3-05 promotion" 참조 superseded).
...
| 필드 | 타입 | 필수 | 소유/출처 | 설명 |
|---|---|---|---|---|
| `recommendation_id` | string = **`rec_v3_` + ULID(26)** — 사전 §1.1 유일 형식 (구 "`rec_v3_`+32" 표기 superseded — 사전 §1.1) | ✔ | Cosmile mint | 이벤트/추천 상관관계 키. downstream(V3-04)이 이 키로 order/feedback을 연결(NULLABLE — 사전 R-K1·`attribution_mode` 사전 §2.9). |
| `event_type` | enum(§2) | ✔ | Cosmile | 이 레코드가 나타내는 상호작용 종류. |
| `subject_ref` | string (`subj_v2_`+32 — 형식 정본 = 사전 §1.1) | 조건부 | service-local mint | 식별된 고객. `HMAC(<SVC>_SUBJECT_SECRET,'<svc>:subject:'+ref)[:32]`. **또는** `anonymous_ref`. |
| `anonymous_ref` | string (`anon_v3_`+opaque 32 — 사전 §1.1) | 조건부 | service-local | 비식별 세션 대상. subject_ref와 **택일**(둘 다 없음 금지, 둘 다 있음 금지 — §5 규칙 R1). **commerce event 계층 전용** — memory 계층(ltm_fact/candidate)의 비로그인 키는 `guest_ref`(사전 §1.1·V3-02)이며 anonymous_ref의 memory 직접 유입 금지. downstream 유지 = 사전 R-K3·stitching = 사전 §2.10. |
...
| `confidence` | float 0.0–1.0 (승격 문턱 정본 = 사전 §3) | ✔ | Foundation reasoning | 추천 근거 확신도. 단일 신호로 장기기억 승격 금지(`COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md`). |
...
**furef 부재 = 정본:** RecommendationEvent에는 `furef` 필드가 **없다**(의도된 부재 — 이벤트/귀속 키는 사전 §1의 키만: `subject_ref XOR anonymous_ref`·`recommendation_id`·`session_id`·`product_id`/`sku_id`). `COSMILE_MEMORY_V3_09_ANALYTICS_REPORT_MINIMUM_20260706.md`의 지표도 사전 키만 사용하며 furef 기반 귀속을 정의하지 않는다.
...
| `recommendation_saved` | 찜/저장/위시. | 관심 신호(MemoryFactCandidate 후보, `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md`). |
...
**저장 소유 단일(사전 R-K7):** 얕은 상호작용(`recommendation_shown`(≡impression)·`recommendation_clicked`·`recommendation_dismissed`·`recommendation_saved`·`recommendation_added_to_cart`)의 **저장 소유 = 본 문서(V3-03) 단일**이다. `COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md`는 checkout 이후 결과(stage ≥ order)만 소유하고, `COSMILE_MEMORY_V3_09_ANALYTICS_REPORT_MINIMUM_20260706.md`는 두 소스를 **읽기만** 한다(자체 이벤트 테이블 정의 금지 — 사전 R-K7).

★Leo 결정 필요: `recommendation_dismissed`를 negative preference 학습 신호로 쓸지 여부.
"닫음 = 싫음"은 휴리스틱 위험(그냥 스크롤일 수 있음) → 의미 판단은 AI semantic, 저장은 후보(candidate)로만. 승격은 `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` gate.
...
**reason_codes 값 목록의 정본 = 사전 §2.15 taxonomy** — 값 선언은 사전만이 하고, 확장은 사전 개정으로만 한다. 아래 표는 값 목록 재선언이 아니라 **도메인 의미 설명**이며, 정본 코드명으로 표기한다(구 표기는 superseded 흔적만).

**reason_codes (추천 근거 — 정본 코드 = 사전 §2.15):**

| code | 의미 |
|---|---|
| `consultation_derived` (구 `consultation_match` — superseded, 사전 §2.15) | 상담에서 표현된 니즈와 일치. |
| `ingredient_match` (구 `ingredient_fit` — superseded, 사전 §2.15) | 성분 축이 사용자 상태/목표에 부합. |
| `concern_match` (구 `skin_goal_alignment` — superseded, 사전 §2.15) | 사용자 목표(진정/보습 등)와 정합. |
| `repurchase_cycle` (구 `repurchase_pattern` reason_code 표기 — superseded, 사전 §2.15) | 과거 재구매/반복 사용 패턴(장기기억, `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` 승격분에 한함). |
| `low_irritation_profile` — **사전 §2.15 미등재·사전 개정 후보**(개정 전 canonical 아님) | 저자극 프로파일 우선. |
| `alternative_to_flagged` — **사전 개정 후보**(사전 `safety_filtered`와의 통합 여부는 사전 개정 시 결정) | safety로 걸러진 상품의 대체 제안. |
| `popular_in_cohort` — **사전 개정 후보** | 코호트 선호(약한 근거, confidence 낮게). |

**avoid_reason (비추천/보류 근거)** — 사전 §2.15에 아직 미등재. canonical 등재는 **사전 개정으로만**(아래 ★Leo 소유처 결정과 함께). safety류 코드의 우선성 규칙은 등재 여부와 무관하게 유효:
...
본 절 enum은 **추천 이벤트 계층**의 후보이며 canonical 등재는 사전 개정으로만. fact/memory 계층의 safety flag 정본 = **사전 §2.13**(`pregnancy_context`는 사전의 `pregnancy_nursing_context`[M2 reused]에 대응 — 계층이 다르므로 혼용 금지).
...
**R1 보충 (P4 — downstream/stitching):** R1(`subject_ref XOR anonymous_ref`)은 유지하되, `anonymous_ref`는 **downstream outcome까지 유지**된다(V3-04 rec_outcome_event·V3-09에 `subject_ref XOR anonymous_ref` 컬럼 쌍 — 사전 R-K3). 로그인 전후 연결은 `identity_stitching_state`(사전 §2.10: `unlinked`→`stitched_pending_consent`→`stitched`)로 표현하며, stitching은 상거래 귀속에만 소급 적용되고 memory promotion에는 consent 확인 후에만 반영된다(사전 §1.3).
...
| `rule_based` | 규칙/필터 기반. |
| `memory_replay` | 장기기억(`COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` 승격분) 기반 재현 추천. |
...
[V3-04] order_event / feedback_event / outcome_event
   ...
   - repurchase          (rec_v3_XXXX → 반복 사용)
             │
             ▼
[V3-06] MemoryFactCandidate → evidence/confidence → LongTermMemoryFact 승격
   - "이 추천이 좋은 결과로 이어졌는가"를 rec_v3_XXXX 체인으로 재구성
   - 단일 신호로 장기기억 확정 금지(candidate → 누적 evidence → promotion)
```

(구 도식의 "[V3-05] 승격" 표기는 superseded — 승격 규칙 문서 = `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md`.)

**연결 계약(요지):**
- V3-04 결과 이벤트는 `recommendation_id`(**NULLABLE** — 추천 없이 발생한 주문도 있으므로, 사전 R-K1)를 optional FK 성격으로 보유. 귀속 구분 = `attribution_mode`(사전 §2.9: `direct`/`session`/`organic`/`unattributed`/`unknown`).
- 추천→결과 매칭은 `recommendation_id` 우선, 없으면 `(subject_ref|anonymous_ref, product_id, session_id, 시간창)` 근접 매칭은 **후보(candidate)로만**(휴리스틱 → 최종 확정 아님, `COSMILE_MEMORY_V3_06_MEMORY_FACT_CANDIDATE_PROMOTION_RULES_20260706.md` gate 경유).
- 매칭 시간창의 정본 초안 = **사전 §4**(purchase attribution 14d·repurchase 90d — ★Leo 확정 파라미터·본 문서는 수치를 직접 선언하지 않는다). **자세한 결과 계약은 `COSMILE_MEMORY_V3_04_ORDER_REVENUE_FEEDBACK_OUTCOME_CONTRACT_20260706.md` 참조.**

★Leo 결정 필요: 추천 없이 발생한 주문(비추천 유입)을 **학습 루프에 포함할지** 여부. (기록 자체는 사전 R-K1 nullable + R-K2 `organic`으로 정본화되어 가능 — 열린 질문은 학습 포함 여부만.)
...
1. ★Leo 결정 필요: reason_codes/avoid_reason/safety_flags enum canonical 소유처(권고: Foundation). — 값 목록의 어휘 정본은 사전 §2.15(확장은 사전 개정으로만).
2. ★Leo 결정 필요: `recommendation_dismissed`를 negative preference 학습 신호로 쓸지.
3. ★Leo 결정 필요: 추천 없이 발생한 주문을 **학습 루프**에 포함할지(기록은 사전 R-K1/R-K2로 가능 — §7).
4. ★Leo 결정 필요: 추천→결과 근접 매칭(시간창) 허용 범위 — 정본 초안 = 사전 §4. 어디까지가 candidate이고 어디부터 discard인지.

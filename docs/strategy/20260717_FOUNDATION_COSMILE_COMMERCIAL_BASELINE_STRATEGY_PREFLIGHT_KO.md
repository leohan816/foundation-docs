...
BOUNDED_CONTINUATION_WITHIN_APPROVED_SCOPE: ALLOWED
```

이 문서는 약 3-working-day를 목표로 하는 commercial baseline audit의 필요성, 범위,
...
약 3 working days를 목표로 하는 read-only commercial baseline audit은 필요하다.
그러나 아래 correction을 고정하기 전에는 활성화하면 안 된다.
...
6. `INVESTIGATION_PACKAGE_COMPLETE`, `AUDIT_COMPLETE`, Paid Beta/Public Launch
   ...
   아니다.
7. 3 working days는 target duration이며 Day 3은 mandatory checkpoint다. Day 3에
   completed evidence를 freeze하고 remaining questions, scope stability, revised bounded
   completion estimate를 기록한다. 필요한 work가 기존 승인 범위 안에서 bounded하면
   별도 Leo 승인 없이 계속하며, Day 3 자체는 hard stop이 아니다.
8. 시작 전 내부 escalation owner와 return route는 필수다. substantive domain
   owner는 승인된 audit을 안전하게 수행하거나 intended decision에 답하는 데 꼭
   필요하고 safe unresolved fallback이 없을 때만 admission dependency다.
...
- 약 3일 target이 production readiness certification 또는 불완전한 조사 종료를
  강제한다는 뜻이 아니다.
...
- “전체 repository surface map”은 파일 전수 line review가 아니라 commercial
  ownership map이어야 한다.
- audit 시작 pin과 최종 commercial branch decision을 구분해야 한다.
...
- 약 3 working days target과 mandatory Day 3 checkpoint, bounded continuation,
  이후 Independent Review/Advisor closure elapsed time을 구분해야 한다.
...
별도 승인될 audit의 목적은 다음 질문에 current-state evidence로 실제 답하는 것으로
제한한다. Day 3은 답변을 중단하는 종료선이 아니라 progress/scope/estimate checkpoint다.
...
- Day 0 P1에는 exact pins, authority/return route, selected actor capacity, evidence
  ...
  P1–P5 mapping, Day 3 checkpoint, bounded-continuation rule, UTC checkpoints와
  return-to-Leo triggers를 기록한다.
...
- Day 2 이후에는 `CONTINUE`된 question만 first decision-relevant/blocking boundary까지
  추적한다. Day 3 이후에도 necessary, bounded, already-approved-scope 조건을 모두
  만족하면 completion까지 계속한다.
...
## TARGET_DURATION_AND_DAY_3_CHECKPOINT

### 현실성 판정

`APPROXIMATELY_3_WORKING_DAYS`는 baseline reconstruction과 P1–P4 decision package의
target duration이다. Day 3은 mandatory checkpoint이며 hard stop이 아니다.
production certification, 외부 계약 완료, exhaustive security/code review, bug fix,
새 architecture 설계까지 포함하면 불가능하다.

실제 current-state confirmation이 Day 3에 끝나지 않았더라도 remaining work가
necessary, bounded, within approved scope이면 계속한다. P5 Independent Review와
Advisor closure는 최종 P1–P4 freeze 이후 별도 elapsed time으로 측정한다.
...
### Day 3 — mandatory checkpoint

- completed evidence와 exact pins를 immutable checkpoint snapshot으로 freeze
- current as-built, mock-versus-real, blocker/debt, Track C/F/X/P의 완료 상태 기록
- 아직 답하지 못한 exact remaining questions와 각 decision impact 기록
- scope가 original approved audit scope에서 확대되지 않았는지 확인
- remaining work가 `NECESSARY`, `BOUNDED`, `WITHIN_APPROVED_SCOPE`인지 판정
- revised bounded completion estimate와 confidence 기록
- `CONTINUE_WITHIN_SCOPE | COMPLETE_AND_FREEZE_FINAL_PACKAGE | RETURN_TO_LEO` 판정

Day 3 snapshot은 이미 완료된 evidence를 보호하지만, 승인 범위 안의 필요한 remaining
evidence collection을 금지하지 않는다.

### Day 3 이후 bounded continuation

- `NECESSARY: YES`, `BOUNDED: YES`, `WITHIN_APPROVED_SCOPE: YES`이면 Advisor가
  기존 audit을 계속 운영한다.
- continuation은 새 feature, architecture design, implementation, full-repo review,
  새로운 risk acceptance로 확대할 수 없다.
- remaining estimate와 실제 progress를 계속 기록하며 완료된 evidence는 append-only
  delta로 freeze한다.
- actual current-state confirmation과 intended decision package가 완성될 때 최종
  P1–P4를 freeze한다.
...
- 이 시간은 investigation elapsed time과 별도로 기록한다.
- P5 중 P1–P4 evidence collection을 다시 열지 않는다.

### Day 3 target 안에 완료를 보장하지 않는 것
...
### Day 3 checkpoint와 return 규칙

Day 3에는 다음 checkpoint를 반드시 작성한다.

```text
TARGET_DURATION: APPROXIMATELY_3_WORKING_DAYS
DAY_3_CHECKPOINT: COMPLETED
HARD_STOP_AT_DAY_3: NO
COMPLETED_EVIDENCE_FROZEN: YES
EXACT_REMAINING_QUESTIONS: <bounded list>
SCOPE_EXPANDED: NO
REMAINING_WORK_NECESSARY: YES | NO
REMAINING_WORK_BOUNDED: YES | NO
WITHIN_ALREADY_APPROVED_SCOPE: YES | NO
REVISED_BOUNDED_COMPLETION_ESTIMATE: <estimate and confidence>
CHECKPOINT_DECISION: CONTINUE_WITHIN_SCOPE | COMPLETE_AND_FREEZE_FINAL_PACKAGE | RETURN_TO_LEO
IMPLEMENTATION: NOT_AUTHORIZED
NEXT_MISSION: NOT_AUTHORIZED
INVESTIGATION_PACKAGE_COMPLETE: YES | IN_PROGRESS_BOUNDED | HOLD
AUDIT_COMPLETE: NO_UNTIL_P5
RETURN_TO_AFTER_P5: foundation-advisor -> Strategy Decision Architect -> Leo
```

Day 3 이후 continuation에는 새 Leo 승인이 필요하지 않다. 단, 다음 중 하나면
`foundation-advisor -> Strategy Decision Architect -> Leo`로 즉시 return한다.

- material scope expansion이 필요하다.
- 새로운 high-risk decision 또는 risk acceptance가 필요하다.
- remaining duration이 original estimate를 materially exceed한다.
- 승인된 audit으로 intended decision에 답할 수 없게 됐다.

단순히 Day 3이 지났거나 bounded remaining question이 있다는 이유만으로 audit을
종료하지 않는다. 목적은 Cosmile과 Foundation의 실제 current state를 충분히 확인해
intended commercial decision에 답하는 것이다.
...
1. **P1 — AUDIT_MANIFEST_AND_CLOSURE_RECORD**
   - exact pins, authority/return route, selected actors/capacity, evidence ceiling,
     optional E3 allowlist, Day 1/Day 3 checkpoint/return rule, UTC checkpoints,
     product diff 0,
     non-authorization
...
| audit 결과 전 implementation 시작 | 최종 P1–P4 + independent review + Advisor audit + Leo 결정 전 금지 |
...
| source 존재를 READY로 과장 | E1/E2와 E3/E4 및 external state를 분리 |
| 오래된 test count 재사용 | exact HEAD에서 승인된 evidence만 current로 인정 |
| current shadow branch를 commercial branch로 자동 채택 | branch baseline을 final decision item으로 유지 |
| SIASIU full audit로 scope 확대 | dependency boundary-only |
| Control이 Team leader처럼 dispatch | 현행 Advisor-only routing 강제 |
| Day 3 이후 무제한 지속 | checkpoint에서 necessity/scope/estimate를 재검증하고 return trigger 강제 |
...
1. **Audit 실행 승인**
   - `TARGET_DURATION: APPROXIMATELY_3_WORKING_DAYS`, mandatory Day 3 checkpoint,
     no hard stop, necessary/bounded/in-scope continuation, 별도 P5로 실행할지.
   - material scope expansion, new high-risk decision, material duration overrun,
     intended-decision failure일 때만 Leo에게 return하는 규칙을 승인할지.
...
6. **Escalation owner와 conditional domain-owner rule**
   ...
   - substantive owner는 `approved audit의 safe evidence 또는 intended decision에
     필요`하고 `safe unresolved fallback이 없음`이 모두 참일 때만 admission
     dependency다.
...
따라서 본 문서의 blocker 언급은 모두 `PRELIMINARY_NOT_COMPLETE`이며, 별도 승인된
actual audit 결과나 independent review verdict를 대신하지 않는다.

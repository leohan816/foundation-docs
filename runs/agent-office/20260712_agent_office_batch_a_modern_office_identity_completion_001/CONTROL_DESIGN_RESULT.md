...
## Production-render contract correction — PR-1/PR-2/PR-3/PR-4 (candidate `2e0dddfcd8131206f63780c7613bc7d1a03f496d`)

Advisor validation `34_ADVISOR_CONTROL_PRODUCTION_RENDER_SCOPE_VALIDATION.md` found the prior `9caff0e` correction still under-specified on three findings; per handoff `09J`, corrected docs-only from direct code evidence (no product decision, no authority change; WU-02/03/04 + U1-U3/S3/R2/T3 + §14.4 preserved).

**Code evidence read this pass (no agents):** `src/runtime/projection.ts` (`LivingOfficePresentationV1` = only `schemaVersion`/`projectionRevision`/`evaluatedAt`/`frame: OrganizationFrame`); `src/application/organization/types.ts` (`OrganizationFrame` = only `actors: OrganizationFrameActor[]` + `diagnostics`; actor carries `roleInstanceId`/`advisorTeam`/`aiRuntimeState`/`operationalState`/`mission`/`workUnit` envelopes) + `projector.ts` (`projectOrganizationFrame`, `mapOperationalState`); `src/ui/pixel/contracts.ts` (`PixelPrototypeProjection` = pods/actors/cues, `PixelPodInput`, `PixelCueInput`, `PixelPrototypeViewOptions`, `PIXEL_WORLD_FRAME_SCHEMA_VERSION` = output-frame); `pixel-world-scene.tsx:19` static `projectPixelWorldFrame` import + `renderer-boundary.tsx:20` imports the scene + `prototype-entry.tsx:26` imports `PixelWorldChunk` with `projection: PixelPrototypeProjection`; `world-layout.ts` (`createPixelWorldLayout(pods)` fixture-free); `vite.config.ts`; both `tests/acceptance/production-*-boundary.test.ts`.

- **PR-1 — claimed production input does not exist → composed versioned contract.** Replaced the false claim (that authenticated `livingOffice` supplies pods/layout/cues/selected Team) with `LivingOfficeProductionRenderInputV1` (`agent-office.living-office-production-render-input.v1`) = `{ schemaVersion, operational: LivingOfficePresentationV1 (RT), committedLayout: CommittedOfficeLayoutConfigV1, viewport, selection }`, plus an exact per-input **source/authority table** (delta §2.3): RT operational truth (actor `roleInstanceId`/`operationalState`/`aiRuntimeState`/`mission`/`workUnit`/`advisorTeam`, diagnostics→cues) vs committed identity/visual config (registry `displayName`/`roleCategory`/`presentationPodId`, `CommittedOfficeLayoutConfigV1` pods, `createPixelWorldLayout` layout, `presentation-clock` clock, committed default camera/selection). Stable identity kept distinct from mutable Team binding; missing/conflicting RT facts fail closed; no field sourced from `fixtures/prototype-*`. Contract §3 updated.
- **PR-2 — renderer call chain still selected the prototype projector → separate production chain, prototype frozen.** Prototype chain `prototype-entry.tsx → pixel-world-chunk → renderer-boundary → pixel-world-scene → frame-projector → fixtures/prototype-*` is frozen and `prototype-entry.tsx` unedited. New parallel production chain (all named in delta §9, no wildcard): `production-pixel-office-chunk.tsx` (sole production Pixi lazy entry) → `production-renderer-boundary.tsx` → `production-pixel-world-scene.tsx` → `projectLivingOfficeFrame` (`production-frame-projector.ts`) → `frame-core.ts`; shared fixture-free `pixel-frame-stage.tsx` (Pixi draw) + `pixel-render-host.tsx` (backend host) by **extraction** (anti-split-brain), with behavior-preserving edits to `pixel-world-scene.tsx`/`renderer-boundary.tsx`. No production module imports `frame-projector.ts`, `fixtures/prototype-*`, `PixelPrototypeProjection`, or `PixelPrototypeViewOptions`.
- **PR-3 — input validation under-specified → runtime validator.** New `src/application/organization/production-render-input.ts` `parseLivingOfficeProductionRenderInput(raw): { ok } | { ok:false; reason; fallbackTier }` branches on `typeof`/enum-membership (a type cast is not validation; `PIXEL_WORLD_FRAME_SCHEMA_VERSION` is output-frame only). Rules: wrong/missing schema → `DOM_STATIC`; duplicate `roleInstanceId` first-wins + diagnostic; invalid/multiple/missing Team → `UNASSIGNED` + diagnostic; invalid state → `UNKNOWN_OR_STALE`; stale/conflict → cue `freshness=STALE`/`conflict=true`; empty/unresolvable pods → `M1_FIXED_STATIONS`; never throws into render. Validation unit test added to the WU (delta §10, WU-01).
- **PR-4 — bundle proof robust to Vite splitting.** CD-3 (delta §10) redefined as four tree-shaking-independent checks: (a) static import-boundary scan of the eager/fallback graph; (b) build-manifest **reference-edge** evidence (`@pixi`/`pixi.js` referenced only from the dynamic-import subtree of `production-pixel-office-chunk`; vendor-splitting allowed — assert edges, not file count); (c) built-output prototype-marker exclusion; (d) production module graph has no edge to `frame-projector.ts`/`fixtures/prototype-*`. Not "@pixi in exactly one file"; no tree-shaking reliance.

Changed docs (four canonical, non-docs 0): delta §2.3 (PR-1/2/3/4) + §9 (named new/edited paths) + §10 (CD-3 PR-4) + §14.6 (closure) + header; contract §3 + header; WU plan BA-WU-01 Part 2 + §3 CD-3 gate + header; FEATURE_INDEX §2.0. Source/tests/config/assets untouched; `prototype-entry.tsx`/`frame-projector.ts`/`fixtures/*` not edited.

`RETURN_TO: Advisor` — Advisor **revalidates** PR-1/PR-2/PR-3 closed by `2e0dddf` before routing the independent SOL Sentinel narrow docs/source-consistency review; Control has stopped and did not implement, review, self-review, accept risk, grant approval, or enter Batch B–E.

---

## Sentinel PRC-1..PRC-8 correction (candidate `e8531a306a28b4f2858a49b32cc2b3c1bfb4ce6d`)

Independent Sentinel delta review (`SENTINEL_PRODUCTION_RENDER_CONTRACT_DELTA_REVIEW_RESULT.md`, verdict `NEEDS_PATCH`) + Advisor validation `37_ADVISOR_SENTINEL_PRODUCTION_RENDER_REVIEW_VALIDATION.md` (`ACCEPT_NEEDS_PATCH__ROUTE_CONSOLIDATED_PRC1_PRC8_TO_SAME_CONTROL`) found eight documentation-contract defects in `2e0dddf`; corrected docs-only from the cited source evidence per handoff `09K`. Findings closed **not erased** (no product/authority/security/risk decision; WU-02/03/04 + U1-U3/S3/R2/T3 + §14.4 preserved).

**Code evidence read this pass (no agents):** `organization/projector.ts:181-183` (advisorTeam ← `registryRow.advisorTeam` via `normalizeAdvisorTeam`, registry provenance; `:203-206` operationalState/mission/workUnit ← RT); `organization/types.ts:26-41` (`OrganizationRegistryRow` fields — no pod/roleCategory/responsibleAdvisor/projectIdentity), `:55-63` (`AI_RUNTIME_UNKNOWN`), `:17` (`OPERATIONAL_STATE_UNKNOWN='UNKNOWN'`), `:149-161` (`OrganizationDiagnostic{code,roleInstanceId,detail}`); `organization/registry.ts:58-80` (`partitionRegistry` drops **all** duplicate-id rows); `contracts.ts:24-40` (`PixelOperationalState` incl. `UNKNOWN`), `:99-107` (`PixelProjectIdentity` 8 fields), `:110-125` (`PixelPodInput` 15 fields), `:130-136` (`roleCategory` 6-value union), `:183-199` (`PixelCueInput`); `client.ts:288-320` (`/api/v1/projection` → `parseProjection`) + `~519-549` (`value as unknown as RuntimeProjectionSnapshot`, livingOffice uncast); `living-office-actor-overlay.tsx:20,83` + `living-office-semantic-mirror.tsx:2,6` (`projection: PixelPrototypeProjection`), `living-office-hud.tsx` (already `{frame,...}`); `world-layout.ts:16-58` (`createPixelWorldLayout` sorts by `projectIdentity.projectId`); `vite.config.ts` (no manifest).

- **PRC-1** — `advisorTeam` relabeled (A)-committed-registry-owned (transport in the RT envelope ≠ RT authority); RT owns only mission/workUnit/activity/operationalState; `roleCategory`/`presentationPodId`/`responsibleAdvisorRoleInstanceId`/`PixelProjectIdentity` (not `OrganizationRegistryRow` fields) get **closed total maps** in `CommittedOfficeLayoutConfigV1`. (delta §2.3 table, contract §3)
- **PRC-2** — committed WU-plan/gate progress removed as a second work truth; no RT pod-progress surface exists, so counts render **fail-closed literal 0**, no pod "complete", `totalGates` + semantics stated, no committed catalog. (delta §2.3 table)
- **PRC-3** — `cues=[]` for Batch A (diagnostics cannot build `PixelCueInput`); diagnostics may suppress motion / render as diagnostics only, never create work/routing/review cues. (delta §2.3 table)
- **PRC-4** — invalid `operationalState`→`UNKNOWN`, invalid `aiRuntimeState`→`AI_RUNTIME_UNKNOWN` (`UNKNOWN_OR_STALE` is `ObservableProjectionName` input only); duplicate `roleInstanceId` → **drop-all / hard-fail deterministically** (matches `registry.ts:58-80`), never first-wins. (delta §2.3 PR-3)
- **PRC-5** — complete versioned `CommittedOfficeLayoutConfigV1` + `CommittedPodConfig` interfaces, canonical source `office-layout-config.ts`, deterministic ordering/selection, closed role/project maps, responsible-Advisor resolution, membership, all `PixelPodInput` fields + fail-closed counts, missing/multiple-assignment → `UNASSIGNED`; no operational ownership, no authority-from-location. (contract §3.1)
- **PRC-6** — two named validation boundaries: `client.ts:parseProjection` validates the raw `livingOffice` subtree before client state (today it casts), then `parseLivingOfficeProductionRenderInput` validates the composed render input before lazy renderer entry (called by `production-pixel-office-chunk.tsx`); nested exact-key/envelope/enum/identity/pod/membership/Advisor/selection/anchor/numeric rules + literal defaults; monotonic clock zero valid (`>= 0`); no cast is validation. (delta §2.3 PR-3, contract §3.1)
- **PRC-7** — `frame-projector.ts` is edited (bounded, behavior-preserving) to consume `frame-core.ts` — stated consistently (earlier "not edited" removed); one fixture-free `LivingOfficeStructuralProjection` props contract for overlay + mirror (HUD already fixture-free), which `PixelPrototypeProjection` structurally satisfies so `prototype-entry.tsx` stays unedited; production imports no prototype type/projector/fixture by source construction. (delta §2.3 PR-2, §9)
- **PRC-8** — CD-3 test uses the **Vite JS API** `build()` with test-only manifest/write capture (committed `vite.config.ts` unchanged), classifies modules via Rollup **`OutputChunk.modules`**, traverses `imports`/`dynamicImports`; single literal validator/layout test path `tests/contract/production-render-input.test.ts` ("or an exact sibling" removed). (delta §10, §9)

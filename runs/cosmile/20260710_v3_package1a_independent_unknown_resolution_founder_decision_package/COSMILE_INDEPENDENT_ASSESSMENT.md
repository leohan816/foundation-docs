...
**SAFE_DEFAULT:** Agree with register: collect nothing, write no `RecOutcomeFeedback`, never treat consultation text as outcome evidence. Additionally: do not add *any* free-text field to order surfaces before U-03/U-05 lineage decisions — the event write path is multi-layer sanitized (corrected, see U-05), but a feedback text box would add prose volume against a denylist whose unlisted-key residual is not yet policy-covered.
...
**COSMILE_POSITION (corrected per C-F1):** Register correct on the Foundation-side unknowns; the Cosmile side adds three directly-evidenced facts. (1) Cosmile's own consultation persistence is deliberately hash/whitelist-only **and unwired**: `ConversationMessage` stores `contentHash` (keyed HMAC, "원문 미저장") and has no writer; meta routes accept no raw-text field. (2) Cosmile masking is narrow: `maskPii` covers email + one KR phone pattern + 1000-char cap only (`cosmileSemanticAdapter.ts:13-17`) — names, addresses, order numbers, health details pass through to Foundation transport. (3) The `/api/events` → `CommerceEvent.propertiesJson` write path is **multi-layer sanitized server-side**: `SENSITIVE_KEYS` blocklist **plus** normalized forbidden-key denylist (`isForbiddenKey`) **plus** value-pattern scan (`scanValueForPii`: email/KR-phone/card) **plus** a 1000-char string cap. Generic raw-text keys `message`, `note`, `query`, `searchQuery`, `raw_text`, `answer_substance` (case/separator-insensitively normalized) are **blocked**, and a test proves `message` rejection. The statically supported **residual** is narrower: property keys not on the denylist (e.g. `text`, `comment`, `feedback`, `description` — verified absent from both lists) can carry prose values through, provided the value matches no email/phone/card pattern and is <= 1000 chars. This is a code-path possibility, **not** observed customer-data persistence (no DB accessed).
**DIRECT_EVIDENCE:**
- [CODE] `/api/events` route body (whitelist eventType, server-derived user, `properties` passed through) `src/app/api/events/route.ts:23-33`; server-side sanitize = `SENSITIVE_KEYS` + `isForbiddenKey` + `scanValueForPii` + 1000-char cap (`src/lib/commerceEventService.ts:5,11-23,51` — the header comment explicitly states direct POSTs bypassing client builders are blocked here).
- [CODE] Normalized denylists: `PII_FORBIDDEN_KEYS` + `RAW_TEXT_FORBIDDEN_KEYS` incl. `message`/`note`/`query`/`searchQuery`/`raw_text`/`answer_substance` (`src/lib/events/piiPolicy.ts:6-19`), normalization at `:29-33`; value patterns email/KR-phone/card (`:22-25,41-47`).
- [TEST] `message` key rejected: `scripts/event-schema-eval.mjs:42` (`forbidden_key:message`).
- [CODE] Residual verified: exact standalone keys `text`, `comment`, `feedback`, `description`, `review`, `body`, `content` appear in **neither** blocklist (grep = 0 in `piiPolicy.ts` and `commerceEvent.ts`).
...
**FACTS:** As above. The event write path enforces PII policy server-side; residual exposure is limited to unlisted prose keys passing the value scan. Foundation/provider/infra retention: [UNVERIFIED] from this repo, by design of the mission.
...
**MISSING_EVIDENCE:** Register list + a policy decision on whether the exact-key denylist + pattern scan is sufficient for feedback-scale free text (unlisted-key prose and non-pattern semantic/health text are not value-detected) — see narrowed Addendum A-C3.
...
**SAFE_DEFAULT:** Agree with register; Cosmile-side addition (corrected): before any feedback feature increases free-text volume, decide whether the existing denylist+pattern-scan layers suffice or whether unlisted-key prose needs additional handling; do not weaken the existing `piiPolicy` layers.
**REVERSIBILITY:** Medium before live feedback; any prose that a client may have sent under an unlisted key would be inside `propertiesJson` and need lineage review — a possibility to check, not an observed fact.
**COST_IF_WRONG:** Register list; PII/security incident is the dominant Cosmile-side risk.
**CONFIDENCE:** HIGH on Cosmile repo surfaces (directly enumerated, corrected); LOW end-to-end (out of repo scope).
...
- **A-C3 (corrected per C-F1; source: `src/app/api/events/route.ts:23-33` + `src/lib/commerceEventService.ts:11-23` + `src/lib/events/piiPolicy.ts:6-19`; discovered during U-05, narrowed 2026-07-10):** The `/api/events` write path IS multi-layer sanitized server-side (SENSITIVE_KEYS + normalized forbidden-key denylist blocking `message`/`note`/`query`/`searchQuery`/`raw_text`/`answer_substance` + email/phone/card value scan + 1000-char cap; `message` rejection is test-proven). Narrowed unknown: arbitrary **unlisted** property keys (e.g. `text`, `comment`, `feedback`, `description` — verified absent from both denylists) can carry non-pattern prose values (<= 1000 chars, no email/phone/card match) into persisted `propertiesJson`. This is a **code-path possibility, not observed customer-data persistence**. Must the raw-text lineage policy cover this residual (e.g. allowlist keys, or semantic/health-text value detection beyond the three patterns) before any feedback feature increases free-text volume?
...
| Generic event ingestion `/api/events` | Yes | **Blocked for listed raw-text keys** (message/note/query/searchQuery/raw_text/answer_substance; + PII value scan + 1000-char cap); residual = unlisted prose keys only (code-path possibility) | optional orderId/product fields | `CommerceEvent.propertiesJson` | events route + commerceEventService:11-23 + piiPolicy:6-19 + eval:42 |
...
`src/app/orders/[orderId]/page.tsx` and order-history UI (any explicit linkage UX) · a future feedback route under `src/app/api/` (none exists) · `src/types/recOutcome.ts` + `prisma/schema.prisma` `RecOutcomeFeedback` (provenance additions would be schema work) · `src/lib/commerceEventService.ts` + `src/lib/events/piiPolicy.ts` + `/api/events` (denylist coverage for unlisted prose keys / allowlist decision — existing multi-layer sanitize must not be weakened) · `src/lib/foundationSignalMapper.ts` + outbox (consent reality, whitelist contract) · `src/lib/memoryCandidate.ts` wiring (promotion gates) · `ConsentRecord` writer (absent today) · `src/lib/mergeGuest.ts` (linking-policy carve-out) · consultation meta/`ConversationMessage` (only if consultation-follow-up ever becomes a product decision).

## Correction log (errata — questions unchanged)

- **C-F1 (2026-07-10, per `15_COSMILE_RESULT_VALIDATION.md`): CLOSED.** The first pass claimed the `/api/events` write path applied only `SENSITIVE_KEYS` and that `message`/`query`/`note` free text could pass. Root cause: the first-pass evidence grep over `commerceEventService.ts` matched specific tokens only and missed lines 5/17-18 (`isForbiddenKey`, `scanValueForPii`). Corrected by full-file read at `e4ed668`: the write path applies `SENSITIVE_KEYS` + normalized forbidden-key denylist (blocking `message`, `note`, `query`, `searchQuery`, `raw_text`, `answer_substance`) + email/phone/card value scan + 1000-char cap, with `message` rejection test-proven (`scripts/event-schema-eval.mjs:42`). Corrected sections: U-05 (position/evidence/facts/missing-evidence/safe-default/reversibility/confidence), U-01 safe-default cross-reference, Addendum A-C3 (narrowed to the unlisted-key residual, labeled code-path possibility), input-path reality table row, future-surfaces inventory, files-read list. The narrowed residual (unlisted prose keys such as `text`/`comment`/`feedback`/`description` passing the value scan) is retained as statically verified (0 hits in both denylists). All unrelated U-01..U-09 findings unchanged. Blindness boundary maintained during correction (only the validation file, own assessment, and Cosmile code were read).

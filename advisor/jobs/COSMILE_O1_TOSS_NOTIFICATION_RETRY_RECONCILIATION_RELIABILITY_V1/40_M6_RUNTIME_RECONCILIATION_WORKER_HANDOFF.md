...
PRE-WIRING R1 GATE — RUN ONCE BEFORE ANY PRODUCT WRITE:
- Confirm BASE, clean/upstream-equal Git state, and absence of app/node_modules and app/.next.
- Create only one ignored temporary symlink: app/node_modules -> /home/leo/Project/Cosmile/app/node_modules. No install/copy/generate/package change.
- Run exactly once from app/: ./node_modules/.bin/tsc --noEmit -p tsconfig.json
- Run exactly once from app/ in a clean non-secret process environment with NEXT_TELEMETRY_DISABLED=1: ./node_modules/.bin/next build
- Do not start or serve the build. No DB/provider/network endpoint may be contacted.
- Remove only the mission-created app/.next and node_modules symlink; prove both absent and Git clean.
- Any compile/build failure, unexpected write, connection attempt, or cleanup failure => HOLD. Do not repair or write M6 code.
...
- Vitest command only: NODE_PATH=/home/leo/Project/Cosmile/app/node_modules /home/leo/Project/Cosmile/app/node_modules/.bin/vitest run scripts/o1_toss_reconciliation_runtime.vitest.ts
...
- Do not rerun typecheck or build. No other test, full suite, generate/validate, install, route/runtime start, shared/production DB, or provider command.
...
STOP_CONDITIONS:
- Any extra path/schema/service, new economic semantic/effect, operator impersonation, identifier exposure, compile/build failure, unsafe dependency/runtime need, contract gap, or scope expansion.

GIT_AND_RETURN:
- Advisor monitors paths and commands. Exact-path staging only; one additive product commit; one non-force push only after every gate passes.

...
- Public truth was seven admitted products. The candidate renders the complete returned array; an eighth card appears only after an eighth item is actually admitted.
...
| `/` Home | Non-production notice, exact admitted count, complete eligible catalog, card detail/wishlist/cart targets | Editorial lead followed immediately by the complete catalog; 4-column desktop `4+3`, 2-column mobile flow | Count derives from rendered items; seven means seven cards; no blank eighth tile or incomplete-product content |
...
| `/account` | Guest/member-neutral identity, Google auth, cart/order links, guest-to-user continuity copy | Quiet identity panel; Google action remains explicit; commerce destinations become high-clarity rows | No PII fabrication; auth and guest-to-user cart merge behavior remain intact |
...
|---|---|---|
| `O1EligibleProductCard` has three separate targets | `CatalogCard` with media/detail link, wishlist button, cart button | Interactive controls never nest; each target is at least 44×44 px |
| `WishlistButton` uses pressed state and optimistic rollback | Heart control with outline/filled state | `aria-pressed`, busy guard, and rollback survive restyling |
| `AddToCartButton` exposes idle/adding/added/error/sold-out | Full-width or card-scale purchase action plus persistent message slot | Success persists with Cart link; generic error sits by the action and receives returned focus |

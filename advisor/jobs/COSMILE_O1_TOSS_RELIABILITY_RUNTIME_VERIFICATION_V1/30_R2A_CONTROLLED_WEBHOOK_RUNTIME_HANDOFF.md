...
6. Start only the existing app with worktree-local `next dev --hostname 127.0.0.1 --port 31079`, `NODE_ENV=development`, O1 runtime enabled, `COSMILE_O1_GOOGLE_AUTH_ENABLED=true` with synthetic non-secret Google configuration only, local Toss substitute enabled, Toss mode `test`, test-shaped synthetic non-secrets, loopback public base URL, and no sandbox one-shot/provider credential. The Google flag exists only to satisfy the already-frozen fail-closed runtime configuration contract; no Google route, console, credential, identity, or network request is authorized. Wait for authoritative TCP readiness.

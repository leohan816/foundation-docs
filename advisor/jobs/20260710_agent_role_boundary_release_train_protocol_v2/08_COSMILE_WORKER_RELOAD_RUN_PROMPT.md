# Cosmile Worker Role Protocol Reload Run Prompt
...
Cosmile Worker 확인.
이 지시문은 기존 Cosmile Worker 세션에서만 실행한다. 새 세션이나 sub-agent를 만들지 말라.
다음 파일을 직접 다시 읽어라:
- ../Cosmile/CLAUDE.md
...
Cosmile Worker가 commerce repo-local actor이고 Foundation/SIASIU authority를 소유하지 않음을 확인하라.

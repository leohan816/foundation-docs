...
READ_AND_EXECUTE: ../foundation-docs/advisor/jobs/20260709_v3_11c2_phase1_ephemeral_fresh_deploy_rehearsal/07_SENTINEL_HANDOFF_PROMPT.md
...
../foundation-docs/advisor/jobs/20260709_v3_11c2_phase1_ephemeral_fresh_deploy_rehearsal/07_SENTINEL_HANDOFF_PROMPT.md

기억이나 요약만 보고 검수하지 말라.
Worker 보고를 신뢰하지 말고 가능한 범위에서 실제 repo/status/evidence를 직접 확인하라.
read-only 검수만 수행하라.
runtime 파일을 수정하지 말라.
파일을 stage/commit/push하지 말라.
real target DB, staging DB, prod/live DB에 접근하지 말라.
secret을 읽지 말라.
prisma migrate deploy를 다시 실행하지 말라.

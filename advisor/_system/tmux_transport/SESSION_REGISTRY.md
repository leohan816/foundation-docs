...
| Foundation Designer | `foundation-designer` | `$23` | 0 | `@23` | 0 | `%23` | `/home/leo/Project/foundation-designer` | `codex` v0.144.3, explicitly launched `gpt-5.6-sol` / `max` | isolated design-only workspace with local `AGENTS.md`; reports to `foundation-advisor`; `ultra` is reserved for a documented high-complexity mission | eligible only for exact Foundation product/system design handoffs after live verification; no implementation or independent review authority |

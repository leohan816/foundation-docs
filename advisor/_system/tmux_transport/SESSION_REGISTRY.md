...
| Agent Office SOL Worker | `agent-office-sol` | `$13` | 0 | `@13` | 0 | `%13` | `/home/leo/Project/agent-office` | `codex` (live PID and version revalidated before each dispatch) | same existing Agent Office Codex session formerly named `agent-office`; pane history confirms `gpt-5.6-sol ultra`; active repo-local role instructions | eligible for exact committed Agent Office SOL Worker mission dispatch after live model, effort, role, workspace, branch, and readiness verification; mission branch is specified by each handoff |

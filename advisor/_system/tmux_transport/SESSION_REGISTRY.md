...
| Foundation SOL Reviewer | `foundation-reviewer-sol` | `$17` | 0 | `@17` | 0 | `%17` | `/home/leo/Project/skill/fable-sentinel` | `codex` (v0.144.1 at onboarding) | user-authorized existing-session addition; live onboarding verified `GPT-5.6 SOL` with `max` effort and the dedicated Sentinel skill workspace | primary cross-repo SOL Sentinel candidate; idle until an exact committed `/fable-sentinel` launcher is dispatched; must not implement, patch, or grant final approval |

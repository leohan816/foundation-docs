...
| Foundation SOL Reviewer | `foundation-reviewer-sol` | `$19` | 0 | `@19` | 0 | `%19` | `/home/leo/Project/foundation-reviewer` | `codex` (v0.144.1 at onboarding) | user-authorized independent SOL Reviewer session; live onboarding verified `GPT-5.6 SOL` with `max` effort in the dedicated neutral reviewer workspace | primary cross-repo SOL Sentinel candidate; idle until an exact committed launcher instructs it to read the applicable Sentinel contract; must not implement, patch, or grant final approval |
